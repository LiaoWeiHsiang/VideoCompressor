...
public actor WaveformProvider {
...
#if canImport(AppKit)
    /// macOS memory-pressure watcher (DISPATCH_SOURCE_TYPE_MEMORYPRESSURE).
    /// AppKit has no memory-warning notification on modern SDKs.
    ...
    public func waveform(for url: URL) async -> [Float]? {
