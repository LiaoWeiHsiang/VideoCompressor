    ...
    public var body: some View {
        ...
        .task {
            do {
                ...
                // LOCAL PATCH (see VENDORED.md #2). Upstream called
                // `saveToPhotoLibrary` here unconditionally. This app lets the user pick
                // the destination afterwards (Photos / Immich / both), so writing a copy
                // into Photos before they have chosen would produce an unwanted duplicate
                // every single export. The host now decides, via `onExport`.
                if exporter.isCompleted,
