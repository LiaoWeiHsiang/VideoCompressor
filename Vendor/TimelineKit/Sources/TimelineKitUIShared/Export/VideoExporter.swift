...
public final class VideoExporter {
    ...
    private func exportToFile(_ result: CompositionResult, config: ExportConfig) async throws -> URL {
        ...
        // LOCAL PATCH (see VENDORED.md #1): pass the source's own bitrate so the encoder
        // ceiling can never exceed it — otherwise an already-compressed clip gets
        // re-encoded upward and the output ends up larger than the input.
        let videoSettings = ExportEncodingProfile.videoOutputSettings(
            for: config,
            renderSize: result.videoComposition.renderSize,
            sourceBitrateHint: ExportEncodingProfile.sourceBitrateHint(for: result.composition)
        )
...
// MARK: - Stage 3: Visual export (TimelineRenderer → AVAssetWriter)
    ...
    private func exportVisualToFile(
        ...
        // 2. Video input
        // LOCAL PATCH (see VENDORED.md #1): see the note at the other call site.
        let videoSettings = ExportEncodingProfile.videoOutputSettings(
    ...
    public func saveToPhotoLibrary(url: URL) async throws {
